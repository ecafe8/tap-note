...
总 PRD 当前没有为字体工具单独分配 `FEAT-*` ID，也没有「所属 sub」列。本 sub 暂时追踪总 PRD v5 的 P1 字体工具条目，具体 FEAT ID 待回写总 PRD 后确定；本命令不创建 `feat-*` 目录。

| 功能 ID | 功能名称 | 目录 | 优先级 | 说明 |
|---|---|---|---|---|
| 待分配 | 字体预设与安装 | `feat-font-presets` | P1 | 固定版本字体清单、下载、校验、许可证复制 |
| 待分配 | 字体检查与配置生成 | `feat-font-config` | P1 | 格式、字符覆盖、变体检查，生成 PDF/DOCX 配置 |
| 待分配 | 字体子集化与资源优化 | `feat-font-subset` | P1/P2 | P1 提供脚本入口和方案，按文档动态子集化可作为后续增强 |
...
- 总 PRD 是否新增 `FEAT-011` 等正式功能 ID，并在功能模块表增加「所属 sub」列。
